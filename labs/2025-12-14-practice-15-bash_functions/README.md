...
Функції в `Bash` корисні тоді, коли скрипт стає великим і потребує повторного використання логіки.

- При деплойментах
  - функції типу `check_config`, `wait_service`, `migrate_db` дозволяють не дублювати код у пайплайнах.
- У моніторингу
  - `check_process`, `check_port`, `restart_service` дають можливість створювати "самовідновлювальні" скрипти.
- Робота з логами
  - `parse_logs`, `rotate_logs`, `compress_old_logs` можуть використовуватись в багатьох процедурах.
- Автоматизація серверних задач
  - `setup_user`, `set_permissions`, `configure_network` також багаторазове використання.
- Резервне копіювання
  - функції `backup_db`, `backup_dir`, `cleanup_old` автоматизують роботу с бекапами.

Функції в `Bash` потрібні для структурування великих скриптів, повторного використання коду й розділення логіки на зрозумілі блоки.